...
_LISTLOGENTRIESREQUEST = _descriptor.Descriptor(
    ...
    fields=[
        _descriptor.FieldDescriptor(
            ...
            is_extension=False,
            extension_scope=None,
            serialized_options=b"\340A\002\372A\034\022\032logging.googleapis.com/Log",
            file=DESCRIPTOR,
        ),
        _descriptor.FieldDescriptor(
            name="filter",
...
WriteLogEntriesRequest = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      entries:
          ...
          /quota-policy>`__ in the past or more than 24 hours in the
          future will not be available when calling ``entries.list``.
          However, those log entries can still be `exported with
          LogSinks <https://cloud.google.com/logging/docs/api/tasks
          /exporting-logs>`__.  To improve throughput and to avoid
          exceeding the `quota limit <https://cloud.google.com/logging
          /quota-policy>`__ for calls to ``entries.write``, you should
          try to include several log entries in this list, rather than
