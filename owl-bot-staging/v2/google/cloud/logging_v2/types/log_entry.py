...
class LogEntry(proto.Message):
    ...
    resource = proto.Field(proto.MESSAGE, number=8,
        message=monitored_resource.MonitoredResource,
    )

    proto_payload = proto.Field(proto.MESSAGE, number=2, oneof='payload',
        message=gp_any.Any,
    )

    text_payload = proto.Field(proto.STRING, number=3, oneof='payload')

    json_payload = proto.Field(proto.MESSAGE, number=6, oneof='payload',
        message=struct.Struct,
    )

    timestamp = proto.Field(proto.MESSAGE, number=9,
        message=gp_timestamp.Timestamp,
    )

    receive_timestamp = proto.Field(proto.MESSAGE, number=24,
        message=gp_timestamp.Timestamp,
    )

    severity = proto.Field(proto.ENUM, number=10,
        enum=log_severity.LogSeverity,
    )

    insert_id = proto.Field(proto.STRING, number=4)

    http_request = proto.Field(proto.MESSAGE, number=7,
        message=glt_http_request.HttpRequest,
    )

    labels = proto.MapField(proto.STRING, proto.STRING, number=11)

    operation = proto.Field(proto.MESSAGE, number=15,
    ...
    source_location = proto.Field(proto.MESSAGE, number=23,
